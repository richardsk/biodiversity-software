...
namespace TapirDotNET.Controls
{
	...
	public partial class TpMappingForm : TpWizardForm
	{
		...
		public override void  HandleEvents()
		{
			...
			if (HttpContext.Current.Request.Form["load_schemas"] != null)
			{
				...
				{
                    string handler = HttpContext.Current.Request.Form["handler"];
                    if (handler != null)
                    {
                        r_local_mapping.LoadNewSchema(HttpContext.Current.Request.Form["load_from_location"], handler);
                    }   
                    else
                    {
                        new TpDiagnostics().Append(TpConfigManager.CFG_DATA_VALIDATION_ERROR, "Please select the format of the additional schema to load", TpConfigManager.DIAG_ERROR);
                        return;
                    }
				}
			}
								...
								if (mapping_type != concept.GetMappingType())
								{
									if (mapping_type == "unmapped")
									{
										...
										else if (fill_namespace == ns)
										{
											...
											mapping.SetLocalType(TpUtils.TYPE_TEXT);
								...
								else
								{
									if (mapping_type == "unmapped")
									{
										...
										else if (fill_namespace == ns)
										{
											// Fill with fixed value mapping
											mapping = new TpConceptMappingFactory().GetInstance("FixedValueMapping");
											((FixedValueMapping)mapping).SetLocalType(TpUtils.TYPE_TEXT);
											((FixedValueMapping)mapping).SetValue("");
										}
									}
									else
			...
			if (Utility.OrderedMap.CountElements(this.mTablesAndColumns) == 1)
			{
				...
				foreach ( DataRow row in ((DataTable)this.mTablesAndColumns[tables[0]]).Rows ) 
				{
					if (Utility.StringSupport.StringCompare(concept.GetName(), row["COLUMN_NAME"].ToString(), false) == 0)
					{
						...
						field_type = TpConfigUtils.GetFieldType(row["DATA_TYPE"].ToString());
    ...
    protected Utility.OrderedMap GetOptions( string id ) 
    {
        ...
        {
            options.Add("", "-- format --");
            options.Add("DarwinSchemaHandler_v1", "Old DarwinCore (tied to DiGIR)");
            options.Add("DarwinSchemaHandler_v2", "New DarwinCore");
            options.Add("CnsSchemaHandler_v1", "CNS text file");
            options.Add("CnsSchemaHandler_v2", "CNS XML file");
        }
