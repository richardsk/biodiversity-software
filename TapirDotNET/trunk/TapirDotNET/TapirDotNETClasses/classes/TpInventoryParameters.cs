...
namespace TapirDotNET 
{
	...
	public class TpInventoryParameters:TpOperationParameters
	{
		...
		public override bool LoadKvpParameters()
		{
            ...
            if (HttpContext.Current.Request["concept"] != null)
			{
                concept = HttpContext.Current.Request["concept"];
			}
			else
			{
                if (HttpContext.Current.Request["c"] != null)
				{
                    concept = HttpContext.Current.Request["c"];
				}
			}
			
			if (HttpContext.Current.Request["tagname"] != null)
			{
				tag_name = HttpContext.Current.Request["tagname"];
			}
			else
			{
				if (HttpContext.Current.Request["n"] != null)
				{
					tag_name = HttpContext.Current.Request["n"];
				}
			}
			...
			if (concept != null)
			{
				if (concept is Utility.OrderedMap)
				{
					...
					foreach ( object concept_id in Utility.TypeSupport.ToArray(concept).Values ) 
					{
                        ...
                        if (tag_name is Utility.OrderedMap && ((Utility.OrderedMap)tag_name)[i] != null && tag_name.ToString().Length > 0)
						{
				...
				else if (concept is System.String)
				{
					...
					if (tag_name != null && !(tag_name is Utility.OrderedMap) && tag_name.ToString().Length > 0)
					{
