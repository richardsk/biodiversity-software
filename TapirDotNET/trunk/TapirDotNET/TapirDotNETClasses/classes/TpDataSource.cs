...
namespace TapirDotNET 
{
	...
	{
        public bool mUseClass = false;
        public string mServerClass;
		...
		public virtual void  LoadFromSession()
        {
            try
            {
                this.mUseClass = bool.Parse(TpUtils.GetVar("useclass", "").ToString());
                this.mServerClass = TpUtils.GetVar("serverclass", "").ToString();
            }
            catch (Exception ex)
            {
            }
		...
		public virtual void  LoadFromXml(string file, XmlDocument xpr)
		{
			...
			if (dsnode != null)
            {
                try
                {
                    this.mUseClass = bool.Parse(dsnode.Attributes["useclass"].Value);
                    this.mServerClass = dsnode.Attributes["serverclass"].Value;
                }
                catch (Exception ex)
                {
                }
		...
		public virtual bool Validate(bool raiseErrors)
		{
			...
			do 
			{
                if (mUseClass)
                {
                    try
                    {
                        ITapirServer cls = TpServerClass.CreateClass(mServerClass);
                        if (cls == null)
                        {
                            if (raiseErrors)
                            {
                                error = "Server class (" + mServerClass + ") could not be created.";
                    ...
                    catch (Exception ex)
                    {
                        if (raiseErrors)
                        {
                            error = "Server class (" + mServerClass + ") could not be created : " + ex.Message;
                            new TpDiagnostics().Append(TpConfigManager.CFG_DATA_VALIDATION_ERROR, error, TpConfigManager.DIAG_ERROR);
                        }
		...
		public virtual string GetXml()
		{
			string xml;
			xml = "<datasource";
            xml += " useclass=\"" + (mUseClass ? "true" : "false") + "\"";
            xml += " serverclass=\"" + TpUtils.EscapeXmlSpecialChars(this.mServerClass) + "\"";
        ...
        public virtual void SetUseClass(bool useClass)
        {
            this.mUseClass = useClass;
        }

        public virtual bool GetUseClass()
        {
            return this.mUseClass;
        }

        public virtual void SetServerClass(string className)
        {
            this.mServerClass = className;
        }

        public virtual string GetServerClass()
        {
            return this.mServerClass;
        }
