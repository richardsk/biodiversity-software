...
namespace TapirDotNET 
{
	...
	public class TpLocalMapping
	{
		...
		public virtual void  SchemasEndElement(TpXmlReader reader)
		{
			...
			if (Utility.StringSupport.StringCompare(reader.XmlReader.Name, "schema", false) == 0)
			{
				ns = this.mCurrentSchema.GetNamespace();
				
				this.mAvailableSchemas[ns] = this.mCurrentSchema;
			}
		...
		public virtual void  StartElement(TpXmlReader reader, Utility.OrderedMap attrs)
		{
			...
			if (Utility.StringSupport.StringCompare(reader.XmlReader.Name, "revision", false) == 0)
			{
                this.mRevision = attrs["rev"].ToString();
            }
			else if (Utility.StringSupport.StringCompare(reader.XmlReader.Name, "schema", false) == 0)
			{
				this.mCurrentSchema = new TpConceptualSchema();
            ...
            else if (Utility.StringSupport.StringCompare(reader.XmlReader.Name, "source", false) == 0)
            {
                this.mCurrentSchema.SetSource(attrs["source"].ToString());
            }
            ...
            else
            {
                if (Utility.StringSupport.StringCompare(reader.XmlReader.Name, "concept", false) == 0)
                {
                    ...
                    if (attrs["type"] != null && attrs["type"].ToString() != "")
                    {                        
                        // Try to fix old types
                        if ( this.mRevision == null || int.Parse(this.mRevision) < 641 )
                        {
                            string handler = this.mCurrentSchema.GetHandler();

                            if ( handler == "DarwinSchemaHandler_v1" )
                            {
                                System.Collections.Stack parts = new System.Collections.Stack(attrs["type"].ToString().Split(':'));

                                // Assuming that the namespace starts with "http:"
                                if ( parts.Count >= 3 )
                                {
                                    string type_name = parts.Pop().ToString();

                                    string ns = Utility.StringSupport.Join(":", new Utility.OrderedMap(parts));                            

                                    type = ns + "#" + type_name;

                                    // Just in case...
                                    type = TpConfigUtils.GetPrimitiveXsdType(type, null);
                                }
                            }
                ...
                else if (Utility.StringSupport.StringCompare(reader.XmlReader.Name, "environmentVariableMapping", false) == 0)
                {
                    this.mCurrentMapping = new TpConceptMappingFactory().GetInstance("EnvironmentVariableMapping");
                    //TODO this.mCurrentMapping.SetResource(this.mResource);
                }
                else if (Utility.StringSupport.StringCompare(reader.XmlReader.Name, "singleColumnMapping", false) == 0)
                {
                    ...
                    {
                        this.mCurrentMapping.SetLocalType(attrs["type"].ToString());
                    }
                }
		...
		public virtual void  EndElement(TpXmlReader reader)
		{
			...
			if (Utility.StringSupport.StringCompare(reader.XmlReader.Name, "schema", false) == 0)
			{
				...
				this.mMappedSchemas[ns] = this.mCurrentSchema;
				
				this.mAvailableSchemas[ns] = null;
			}
			else
			{
				if (Utility.StringSupport.StringCompare(reader.XmlReader.Name, "concept", false) == 0)
		...
		{
			string xml;
			xml = "\t<mapping rev=\"" + TpConfigManager.TP_REVISION + "\">\n";
