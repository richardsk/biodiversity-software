...
namespace TapirDotNET 
{
	...
	public class TpInventoryResponse:TpResponse
	{
		...
		public override void  Body()
		{
			...
			sql_builder.AddCountColumn(mRequest.GetCount());
			...
			TpUtils.WriteUTF8(HttpContext.Current.Response, "\n<concepts>");
			TpUtils.WriteUTF8(HttpContext.Current.Response, concepts_xml);
			TpUtils.WriteUTF8(HttpContext.Current.Response, "\n</concepts>");
			...
			{				
				...
				if (this.mRequest.GetCount())
				{
					TpUtils.WriteUTF8(HttpContext.Current.Response, " count=\"" + result_set.Tables[0].Rows[num_recs][num_concepts] + "\"");
				}
				...
				for (int i = 0; i < num_concepts; ++i)
				{
					...
					TpUtils.WriteUTF8(HttpContext.Current.Response, TpServiceUtils.EncodeData(result_set.Tables[0].Rows[num_recs][i].ToString(), db_encoding.ToString()));
										
					TpUtils.WriteUTF8(HttpContext.Current.Response, "</" + tag_names[i].ToString() + ">");
				}
			...
			if (num_recs < result_set.Tables[0].Rows.Count)
			{
				...
				TpUtils.WriteUTF8(HttpContext.Current.Response, " next=\"" + next.ToString() + "\"");
			}
			
			this.mTotalReturned = num_recs;
			
			TpUtils.WriteUTF8(HttpContext.Current.Response, " totalReturned=\"" + num_recs.ToString() + "\"");
			
			if (this.mRequest.GetCount())
			{
				TpUtils.WriteUTF8(HttpContext.Current.Response, " totalMatched=\"" + matched.ToString() + "\"");
			}
