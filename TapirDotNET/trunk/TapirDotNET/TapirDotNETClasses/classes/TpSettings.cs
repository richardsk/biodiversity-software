...
namespace TapirDotNET 
{
	...
	public class TpSettings
	{
        ...
        public string mModifier;            // table.field that will update date last modified value
        public string mModifierDatatype;    // local datatype of mModifier field 
		...
		public virtual void  LoadDefaults(bool force)
		{
			...
			this.mModifier = "";
            this.mModifierDatatype = "";
			
			this.mIsLoaded = true;
		}// end of member function LoadDefaults
		
		public virtual void  LoadFromSession(System.Data.OleDb.OleDbConnection cnn)
		{
            ...
            string[] parts = mModifier.Split('.');
            do
            {
                if ( parts.Length != 2 ) // table.column
                {
                    string msg = "dateLastModified setting could not be split into table/column";
                    //TpDiagnostics::Append( DC_CONFIG_FAILURE, $msg, DIAG_WARN );
                    break;
                }

                
				System.Data.DataTable tables = cnn.GetOleDbSchemaTable(System.Data.OleDb.OleDbSchemaGuid.Tables, new object[]{null,null,null,"TABLE"});
                if (tables.Columns.Count == 0)
                {
                    string msg = "Could not get database metadata for dateLastModified setting";
                    //TpDiagnostics::Append( DC_CONFIG_FAILURE, $msg, DIAG_WARN );
                    break;
                }
			...
			{			
				if (node.Attributes.Count > 0)
				{
					if (node.Attributes["fromField"] != null)
					{
                        ...
                        {
                            this.mModifierDatatype = node.Attributes["datatype"].Value;
                        }
					}
		...
		public virtual bool Validate(bool raiseErrors)
		{
            ...
            if ( this.mModifier != null && this.mModifier != "" && this.mModifierDatatype != null && this.mModifierDatatype != "" && 
             ( this.mModifierDatatype != TpUtils.TYPE_DATETIME &&
               this.mModifierDatatype != TpUtils.TYPE_TEXT ) )
            {   
                if ( raiseErrors )
                {
                    error = "Unsupported datatype for date last modified field (" +
                             this.mModifierDatatype  + "). It must be either " + TpUtils.TYPE_DATETIME + 
                             " or " + TpUtils.TYPE_TEXT;
                    new TpDiagnostics().Append( TpConfigManager.CFG_DATA_VALIDATION_ERROR, error, TpConfigManager.DIAG_ERROR );
                }
		...
		public virtual string GetConfigXml()
		{
			...
			if (!Utility.VariableSupport.Empty(this.mModifier))
			{
                ...
                if ( this.mModifierDatatype != null )
                {
        ...
        public virtual string GetModifierDatatype()
        {
