namespace TapirDotNET 
{
	...
	public class TpConcept
	{
		...
		public virtual string GetConfigXml()
		{
			...
			xml = "\t\t\t";
			xml += "<concept id=\"" + TpUtils.EscapeXmlSpecialChars(this.GetId().ToString()) + "\" " + 
                    "name=\"" + TpUtils.EscapeXmlSpecialChars(this.GetName().ToString()) + "\" " + 
                    type_str + doc_str + 
                    "required=\"" + required + "\" " + 
		...
		{
			string searchable = (this.IsSearchable())?"true":"false";
            string datatypeXml = "";
			string xml;			
			
            if ( this.IsMapped() )
            {
                TpConceptMapping mapping = this.GetMapping();
                string datatype = mapping.GetLocalXsdType();

            if ( datatype != null )
            {
                datatypeXml = " datatype=\"" + datatype + "\"";
            }
        }

			xml = "\t\t\t";
			xml += "<mappedConcept searchable=\"" + searchable + "\" id=\"" + TpUtils.EscapeXmlSpecialChars(Utility.TypeSupport.ToString(this.GetId())) + "\" " +
                datatypeXml + "/>\n";
