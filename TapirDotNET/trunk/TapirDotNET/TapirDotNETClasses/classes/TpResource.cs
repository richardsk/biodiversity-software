...
namespace TapirDotNET 
{
	...
	public class TpResource
	{
		...
		public virtual bool SaveLocalMapping(bool updateResources)
		{
			...
			//capabilities file			
            content = GetCapabilitiesXml(mLocalMapping.GetCapabilitiesXml());
		...
		public string GetCapabilitiesXml(string capMappingXml)
		{
			string xml = "";
			try
			{
			...
			xml = xml.Replace("[MAPPING_XML]", capMappingXml);
		...
		public virtual string GetDateLastModified()
		{
			...
			if (this.mDataSource.Validate(raise_errors))
			{
                ...
                string target = TpSqlBuilder.GetSqlName(modifier);
                mod_data_type = mSettings.GetModifierDatatype();

                if (mod_data_type == TpUtils.TYPE_DATETIME)
                {
                    //TODO ?? target = cn->SQLDate( 'Y-m-d H:i:s', $target );
                }
