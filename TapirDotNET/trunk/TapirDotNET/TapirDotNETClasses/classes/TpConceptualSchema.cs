namespace TapirDotNET 
{
	...
	public class TpConceptualSchema
	{
		...
		{
			return (this.mLocation == null ? this.mSource : this.mLocation);
		}// end of member function GetLocation

        public virtual string GetSource()
        {
            return (this.mSource == null ? this.mLocation : this.mSource);
        }
		...
		{
            string source = "";
            if (mSource != null) source = " source=\"" + TpUtils.EscapeXmlSpecialChars(mSource) + "\"";

			string xml;
			xml = "\t\t";
			xml += "<schema namespace=\"" + TpUtils.EscapeXmlSpecialChars(this.mNamespace) + "\" " + 
                "location=\"" + TpUtils.EscapeXmlSpecialChars(Utility.TypeSupport.ToString(this.GetLocation())) + "\" " + 
