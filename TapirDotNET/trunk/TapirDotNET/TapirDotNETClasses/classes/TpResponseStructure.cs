	...
	public class TpResponseStructure
	{
		...
		public virtual void  StartElement(TpXmlReader reader, Utility.OrderedMap attrs)
		{
			...
			else if (Utility.StringSupport.StringCompare(name, "complextype", false) == 0)
			{
                ...
                if ( attrs["mixed"] != null && attrs["mixed"].ToString().ToLower() == "true" )
                {
                    mixed = true;
                }
						
				xs_complex_type = new XsComplexType(name, ns, defNs, is_global, mixed);
			...
			else if (Utility.StringSupport.StringCompare(name, "sequence", false) == 0 || Utility.StringSupport.StringCompare(name, "all", false) == 0 || Utility.StringSupport.StringCompare(name, "choice", false) == 0)
			{
				...
				else if (new Utility.OrderedMap("sequence", "all", "choice").Search(in_tags[depth - 2]) != null)
				{
						TpLog.debug("Adding particle \"" + name + "\" to current object");
								
						((XsModelGroup)r_current_object).AddParticle(xs_model_group);
                }
                else if (in_tags[depth - 2].ToString().ToLower() == "extension")
                {
                    TpLog.debug("Adding extended content type \"" + name + "\" to current object");
        ...
        */
        public string GetPrimitiveXsdType(XsType type)
        {
            if (type == null) return null;

            string xsd_namespace = "http://www.w3.org/2001/XMLSchema";
            string type_name = type.GetName();
            string type_ns = type.GetTargetNamespace();
            ...
            {
                object base_type = type.BaseType;

                if (base_type is string)
                {
                    base_type = this.GetType(type_ns, base_type.ToString());

                    if (base_type != null)
                    {
                        return this.GetPrimitiveXsdType((XsType)base_type);
                    }
                }
                else if (base_type != null)
                {
                    return GetPrimitiveXsdType((XsType)base_type);
                }
            }
