	...
	public class TpFilter
	{
		...
		public virtual Utility.OrderedMap _ResolveLiterals(string filterString)
		{
			...
			for (i = 0; i < filterString.Length; ++i)
			{
				...
				if (char_Renamed == '\"' && !escaped)
				{
					...
					{
						inside_quote = false;
						tokens.Push(new TpExpression(EXP_LITERAL, literal_content, false));
					}
					else
					{
						inside_quote = true;
				...
				if (part.GetType() == typeof(string))
				{
					...
					foreach ( string token in Utility.RegExPOSIXSupport.Split(" ", part.ToString(), - 1, true).Values ) 
					{
						...
						if (tok.Length > 0 && this.mOperators.GetKeysOrderedMap(null).Search(tok.ToLower()) != null)
						{
							...
							// add concept
                            concept = new TpExpression(EXP_CONCEPT, tok, false);
		...
		*/
										...
										else
										{
											...
											else
											{
												if (Utility.StringSupport.StringCompare(name, "concept", false) == 0)
												{
													...
													{
														((TpComparisonOperator)current_operator).SetExpression(new TpExpression(EXP_CONCEPT, attrs["id"], false));
													}
												}
												else
												{
													if (Utility.StringSupport.StringCompare(name, "t_concept", false) == 0)
													{
														if (this.mIsLocal)
														{
															...
															{
																concept = new TpTransparentConcept(attrs["table"].ToString(), attrs["field"].ToString(), attrs["type"].ToString());

                                                                ((TpComparisonOperator)current_operator).SetExpression(new TpExpression(EXP_COLUMN, concept, false));
															}
														}
														else
														{
													...
													else
													{
														if (Utility.StringSupport.StringCompare(name, "literal", false) == 0)
														{
															...
															{
                                                                ((TpComparisonOperator)current_operator).SetExpression(new TpExpression(EXP_LITERAL, attrs["value"], false));
															}
														}
														else
														{
															if (Utility.StringSupport.StringCompare(name, "parameter", false) == 0)
															{
																...
																{
                                                                    ((TpComparisonOperator)current_operator).SetExpression(new TpExpression(EXP_PARAMETER, attrs["name"], false));
																}
															}
															else
															{
		...
		public virtual void  TestUrlFilterParser(string filterString)
		{
			...
			TpUtils.WriteUTF8(HttpContext.Current.Response, "<br/>String is: " + filterString + "<br/>");
			
			list = this._ResolveLiterals(filterString.Trim(new char[]{' ', '\t', '\n', '\r', '0'}));
			
			TpUtils.WriteUTF8(HttpContext.Current.Response, "<br/><b>After ResolveLiterals</b><br/>");
			...
			foreach ( object el in list.Values ) 
			{
				...
				TpUtils.WriteUTF8(HttpContext.Current.Response, "<br/><b>" + i + ":</b>&nbsp;");
				
				if (el != null)
				{
					class_Renamed = el.GetType().Name;
					
					TpUtils.WriteUTF8(HttpContext.Current.Response, "class (" + class_Renamed + "): " + ((TpExpression)el).GetReference().ToString());
				}
				else
				{
					TpUtils.WriteUTF8(HttpContext.Current.Response, "string: " + el);
				}
			}
			...
			TpUtils.WriteUTF8(HttpContext.Current.Response, "<br/><br/><b>After ResolveConcepts</b><br/>");
			...
			TpUtils.WriteUTF8(HttpContext.Current.Response, "<br/><br/><b>After ResolveBrackets</b><br/>");
			TpUtils.WriteUTF8(HttpContext.Current.Response, "<pre>");
			TpUtils.WriteUTF8(HttpContext.Current.Response, nested_list.ToString());
			TpUtils.WriteUTF8(HttpContext.Current.Response, "</pre>");
			
			nested_list = this._TokenizeData((TpNestedList)nested_list);
			
			TpUtils.WriteUTF8(HttpContext.Current.Response, "<br/><br/><b>After TokenizeData</b><br/>");
			TpUtils.WriteUTF8(HttpContext.Current.Response, "<pre>");
			TpUtils.WriteUTF8(HttpContext.Current.Response, nested_list.ToString());
			TpUtils.WriteUTF8(HttpContext.Current.Response, "</pre>");
			...
			TpUtils.WriteUTF8(HttpContext.Current.Response, "<br/><br/><b>Log representation</b><br/>");
			TpUtils.WriteUTF8(HttpContext.Current.Response, ((TpBooleanOperator)root_boolean_operator).GetLogRepresentation());
