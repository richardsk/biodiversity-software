...
namespace TapirDotNET 
{
	...
	public class TpXmlGenerator:TpSchemaVisitor
	{
		...
		public override object VisitElementDecl(IXsTypedObject rXsElementDecl, object path) //path = string
		{
            ...
            if (r_type == null)
            {
                string msg = "Error processing element " + path.ToString() + " (no type)";
                new TpDiagnostics().Append(TpConfigManager.DC_GENERAL_ERROR, msg, TpConfigManager.DIAG_FATAL);
                this.mAbort = true;
                return "";
            }
			...
			if (!this.mOmitNamespaces)
			{
				...
				if (is_root)
				{
                    if ( target_namespace == "http://www.w3.org/1999/02/22-rdf-syntax-ns#" )
                    {
                        ...
                        System.Web.HttpContext.Current.Response.ContentType = "application/rdf+xml; charset=utf-8";
                    }
					...
					for ( int x = 0; x < ns_to_declare.Count; ++x )
					{
                        ...
                        {
                            if (!Utility.VariableSupport.Empty(pref) || !Utility.VariableSupport.Empty(target_ns_prefix))
                            {
                                xmlns += " xmlns";
                                if (!Utility.VariableSupport.Empty(pref))
                                {
                                    xmlns += ":" + pref;
                                }
                                xmlns += "=\"" + uri + "\"";
			...
			while (num_attempts < limit)
			{
				...
				if (((XsType)r_type).IsComplexType())
				{
					...
					else
					{
                        ...
                        if ( ((XsComplexType)r_type).IsMixed() && current_subcontent_xml == ""  )
                        {
                            TpLog.debug( "Is mixed" );

                            string value = this._GetSimpleContent( rXsElementDecl, (string)path );
		...
		public virtual string _GetSimpleContent(IXsTypedObject obj, string path)
		{
			//global $g_dlog
			string val;
            ...
            {
                val = obj.GetFixedValue().ToString();
            }
			else
			{
				mapping = Utility.TypeSupport.ToArray(this.mOutputModel.GetMappingForNode(path, false));
				
				if (mapping != null)
				{
                    object primitive_type = null;

                    object type = obj.GetType();

                    if ( type != null )
                    {
                        object p_obj = TpConfigUtils.GetPrimitiveXsdType( type.ToString(), null );

                        if ( p_obj != null )
                        {
                            primitive_type = ((XsSimpleType)p_obj).GetUri();
                        }
                    }
					val = this._GetMappedData(mapping, path);
				}
				
				if (val == "" && obj.HasDefaultValue())
				{
					val = obj.GetDefaultValue().ToString();
				}
			}
		...
		public virtual string _GetMappedData(Utility.OrderedMap mapping, string path)
		{
			...
			for (i = 0; i < Utility.OrderedMap.CountElements(mapping); ++i)
			{
				...
				else
				{
					...
					{
						string c = expression.GetValue(this.mrResource, null, false, false, null);
						if (expression.XmlEncode) data += TpUtils.EscapeXmlSpecialChars(c);
						else data += c;
					}
					else
