...
namespace TapirDotNET 
{
	...
	public class TpOutputModel
	{
		...
		public virtual bool Parse(string location)
		{
			...
			{
                error = "Could not import content from XML file: " + ex.Message + " : " + ex.StackTrace;
        ...
        public virtual void StartElement(TpXmlReader reader, Utility.OrderedMap attrs)
        {
            ...
            // <structure>
            else if (Utility.StringSupport.StringCompare(name, "structure", false) == 0)
            {
                // nothing
            }
            // <indexingElement>
            else if (Utility.StringSupport.StringCompare(name, "indexingElement", false) == 0)
            {
            ...
            // <mapping>
            else if (Utility.StringSupport.StringCompare(name, "mapping", false) == 0)
            {
                    ...
                    if (Utility.StringSupport.StringCompare(name, "concept", false) == 0)
                    {
                        if (this.mCurrentMappingPath != null && attrs["id"].ToString() != "")
                        {
                            ...
                            if (attrs["required"] != null &&
                                (attrs["required"].ToString().ToLower() == "true" || attrs["required"] == "1"))
                            {
                                required = true;
                            }

                            ((Utility.OrderedMap)this.mMapping[this.mCurrentMappingPath]).Push(new TpExpression(TpFilter.EXP_CONCEPT, attrs["id"], required));
                        }
                    }
                    else if (Utility.StringSupport.StringCompare(name, "literal", false) == 0)
                    {
                        if (this.mCurrentMappingPath != null && attrs["value"].ToString() != "")
                        {
                            ((Utility.OrderedMap)this.mMapping[this.mCurrentMappingPath]).Push(new TpExpression(TpFilter.EXP_LITERAL, attrs["value"], false));
                        }
                    }
                    else if (Utility.StringSupport.StringCompare(name, "variable", false) == 0)
                    {
                        if (this.mCurrentMappingPath != null && attrs["name"].ToString() != "")
                        {
                            ...
                            ((Utility.OrderedMap)this.mMapping[this.mCurrentMappingPath]).Push(new TpExpression(TpFilter.EXP_VARIABLE, attrs["name"], required));
                        }
                    }
                }
                // inside <structure>
		...
		public virtual object GetMappingForNode(string path, bool addIfMissing)
		{
            ...
            if (this.mAutomapping && addIfMissing)
            {
                string ns = this.mResponseStructure.GetTargetNamespace();

                string path_without_prefixes = System.Text.RegularExpressions.Regex.Replace(path, "/[^/]*:", "/");
                //ereg_replace( '/[^/]*:', '/', $path );

                string concept_id = ns + path_without_prefixes;

                TpExpression expression = new TpExpression(TpFilter.EXP_CONCEPT, concept_id, false);

                Utility.OrderedMap map = new Utility.OrderedMap();
                this.mMapping[path] = map;
                map.Push(expression);
		...
		public virtual bool IsValid()
		{
			...
			if (Utility.OrderedMap.CountElements(this.mMapping) == 0 && !mAutomapping)
			{
