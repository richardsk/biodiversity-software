...
namespace TapirDotNET 
{
	...
	public class TpSearchResponse:TpResponse
	{
		...
		public override void  Body()
		{
			...
			schema_inspector = new TpSchemaInspector(r_resource, output_model, r_local_mapping, max_levels, partial);
			...
			foreach ( string path in output_model.GetMapping().Keys ) 
			{
				...
				if (accepted_paths.GetKeysOrderedMap(null).Search(path) != null)
				{
                    ...
                    int num_mappings = Utility.TypeSupport.ToArray(expressions).Count;
					...
					foreach ( TpExpression expression in Utility.TypeSupport.ToArray(expressions).Values ) 
					{
						if (expression.GetType() == TpFilter.EXP_CONCEPT)
						{
                            ...
                            {
                                TpLog.debug("Skipping target (concatenation): " + concept_id);
                            }
						}
					}
			...
			TpUtils.WriteUTF8(HttpContext.Current.Response, main_content);
			...
			if (this.mRequest.GetEnvelope())
			{
				...
				TpUtils.WriteUTF8(HttpContext.Current.Response, " totalReturned=\"" + this.mTotalReturned.ToString() + "\"");
