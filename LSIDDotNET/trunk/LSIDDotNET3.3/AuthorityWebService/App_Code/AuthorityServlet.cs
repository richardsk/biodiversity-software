		...
		public override void processGet(HttpContext req, LSIDRequestContext rc) 
		{
			...
			{ 
				...
				try 
				{
                    ...
                    if (req.Request.ContentType == HTTPConstants.HTML_CONTENT || req.Request.ContentType == "")
                    {
                        req.Response.ContentType = HTTPConstants.XML_CONTENT; //so that it is viewable in most browsers
                    }
                    else
                    {
                        req.Response.ContentType = HTTPConstants.RDF_CONTENT; 
                    }
